...
## Prerequists
1. Choco install with Administrator Command Prompt `ctrl + shift enter`
    ```shell
    @powershell -NoProfile -ExecutionPolicy Bypass -Command "iex ((new-object net.webclient).DownloadString('https://chocolatey.org/install.ps1'))" && SET PATH=%PATH%;%ALLUSERSPROFILE%\chocolatey\bin
    ```
1. Close and reopen Administrator Command Prompt `ctrl + shift enter`
...
2.  Clone the repository to a local directory 
    ```shell
    git clone https://github.com/aptera/thatsfine.git c:\repos\thatsfine
    ```
...
6. To run the app, execute the following command:
  ...
  gulp watch
  ```
  May have to allow traffic through your firewall.
